    ...
    <div class="block_content">
        <ul class="tree dhtml">
            ...
            <li> 
                ...
                @if(isset($brandAll))
                <ul>
                    @foreach($brandAll as $cc)
                    <li>
                        <a 
                            @if(isset($bid))
                                @if(($catAr->id==$cid) && ($cc->id==$bid))
                                    class="selected"
                                @endif
                            @endif 
                            href="{{url('category/brand/'.$cc->id.'/'.$catAr->id.'/'.$cc->name)}}" title="{{$cc->name}}"> 
                            {{$cc->name}} 
                        </a>
                        @if(isset($relSub))
                        <ul>
                            @foreach($relSub as $rcc)
                            <li>
                                <a 
                                    @if(isset($bid))
                                        @if(($catAr->id==$cid) && ($cc->id==$bid) && ($rcc->id==$scid))
                                            class="selected"
                                        @endif
                                    @endif 
                                    href="{{url('category/brand/'.$rcc->id.'/'.$catAr->id.'/'.$rcc->name)}}" title="{{$rcc->name}}"> 
                                    {{$rcc->name}} 
                                </a>
                            </li>
                            @endforeach
